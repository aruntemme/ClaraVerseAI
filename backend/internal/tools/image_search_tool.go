...
func executeImageSearch(args map[string]interface{}) (string, error) {
	...
	// Handle comma-separated URLs (take the first one) - SEARXNG_URL may contain multiple URLs
	searxngURL := os.Getenv("SEARXNG_URL")
	if searxngURL == "" {
		searxngURL = "http://localhost:8080"
	} else if idx := strings.Index(searxngURL, ","); idx != -1 {
		// Extract first URL if comma-separated list is provided
		searxngURL = strings.TrimSpace(searxngURL[:idx])
	}
