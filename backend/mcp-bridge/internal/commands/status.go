...
func runStatus(cmd *cobra.Command, args []string) error {
	...
	fmt.Println("📊 ClaraVerse MCP Client Status")
	fmt.Println()
	...
	if cfg.AuthToken != "" {
		...
		fmt.Println("   Run 'mcp-client login' to authenticate")
	}
	fmt.Println()
	...
	if cfg.AuthToken == "" {
		...
		fmt.Println("1. Login: mcp-client login")
	} else if len(enabledServers) == 0 {
		fmt.Println()
		fmt.Println("Next steps:")
		fmt.Println("1. Add servers: mcp-client add <name> --path <server-path>")
	} else {
		fmt.Println()
		fmt.Println("✅ Ready to start!")
		fmt.Println("   Run: mcp-client start")
	}
