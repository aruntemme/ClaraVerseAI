...
import type { PromptMessageHandle } from '@/components/chat/InteractivePromptMessage';
...
export const Chat = () => {
  ...
  const [promptAnswersValid, setPromptAnswersValid] = useState(false);

  // Refs
  const promptMessageRef = useRef<PromptMessageHandle>(null);

  // Debug validation state changes
  useEffect(() => {
    console.log('✅ Submit button state:', { promptAnswersValid, activePrompt: !!activePrompt });
  }, [promptAnswersValid, activePrompt]);
  ...
  const toggleThinkingPane = useCallback((messageId: string) => {
  ...
  // Auto-expand thinking pane while streaming, auto-collapse when complete
  useEffect(() => {
    ...
    if (lastMessage?.role === 'assistant' && lastMessage.reasoning) {
      ...
      // Note: Auto-collapse removed - thinking pane will be hidden after completion
    }
  }, [chat?.messages]);
  ...
  useEffect(() => {
    messages.forEach(msg => {
      if (msg.reasoning && msg.role === 'assistant' && msg.isStreaming) {
        // Find the thinking content element and scroll to bottom
  ...
  // Check if user has set their name, show modal if not (for anonymous/non-auth users)
  useEffect(() => {
    // Only show name modal if user is NOT authenticated and hasn't set a name
    if (!user && !hasUserName()) {
  ...
  // Show privacy modal for authenticated users who haven't set it
  useEffect(() => {
    // Show privacy modal if user is authenticated but hasn't set privacy mode
    // Also show if user has set their name but no privacy mode
    ...
    const unsubscribe = websocketService.onMessage((message: ServerMessage) => {
      switch (message.type) {
        ...
        case 'stream_chunk':
          ...
          {
            // Clear context optimization modal if still showing (safety net)
  ...
  const handleToggleThinking = useCallback(
    (messageId: string) => {
      toggleThinkingPane(messageId);
      // Scroll to the thinking pane
      setTimeout(() => {
        const thinkingElement = document.querySelector(`[data-thinking-id="${messageId}"]`);
        if (thinkingElement) {
          thinkingElement.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        }
      }, 100);
    },
    [toggleThinkingPane]
              ...
              <div className={styles.chatPanel}>
                {!hasStartedChat ? (
                  ...
                  <CommandCenter
                    ...
                    onSubmitPromptAnswers={() => promptMessageRef.current?.handleSubmit()}
                    onSkipPrompt={() => {
                      if (activePrompt) {
                          ...
                          <InteractivePromptMessage
                            ...
                            onSubmit={answers => {
                              if (activePrompt) {
                                ...
                                // Update store (adds to history and clears prompt)
                                submitPromptResponse(activePrompt.promptId, answers);
                              }
                            }}
                            onValidationChange={isValid => {
                              console.log('📥 Received validation change in Chat.tsx:', isValid);
                              setPromptAnswersValid(isValid);
                            }}
                          />
                        )}
