...
function createEmptyWorkflow(): Workflow {
  const startBlockId = `start-${Date.now()}`;
  const startBlockName = 'Start';
  return {
    id: generateId(),
    blocks: [
      {
        id: startBlockId,
        normalizedId: normalizeBlockName(startBlockName),
        type: 'variable' as const,
        name: startBlockName,
        description: 'Workflow input - receives trigger data',
        config: {
          type: 'variable' as const,
    ...
    connections: [],
    variables: [{ name: 'input', type: 'object' as const, defaultValue: {} }],
    version: 1,
  };
}
    ...
    persist(
      (set, get) => ({
        ...
        onEdgesChange: _changes => {
          // Handle edge changes from React Flow (deletion, etc.)
          // For now, we sync edges from workflow state
        },
        ...
        onConnect: connection => {
          const newConnection: Connection = {
            ...
            sourceOutput: 'output', // Default output name
        ...
        saveCurrentAgent: async (createVersion = false, versionDescription = '') => {
          ...
          try {
            ...
            // Reload versions from backend if a new version was created
            if (createVersion) {
        ...
        loadAgentFromBackend: async agentId => {
          ...
          try {
            ...
            if (agentResponse) {
              ...
              // If no blocks, create empty workflow with Start block
              if (workflow.blocks.length === 0) {
                const emptyWorkflow = createEmptyWorkflow();
                workflow.blocks = emptyWorkflow.blocks;
                workflow.variables = emptyWorkflow.variables;
              }
