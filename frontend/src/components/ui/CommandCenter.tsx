...
export interface CommandCenterProps {
  ...
  /** Active prompt for answer mode */
  activePrompt?: ActivePrompt | null;
  /** Callback to submit prompt answers */
  onSubmitPromptAnswers?: () => void;
  /** Callback to skip prompt */
  onSkipPrompt?: () => void;
  /** Whether prompt answers are valid (disable submit if false) */
  promptAnswersValid?: boolean;
}
  ...
  (
    {
      ...
      activePrompt = null,
      onSubmitPromptAnswers,
      onSkipPrompt,
      promptAnswersValid = false,
    },
    ref
  ) => {
    ...
    useEffect(() => {
      if (initialValue && initialValue.trim()) {
        ...
        // Notify parent that the initial value has been consumed
        onInitialValueConsumed?.();
      }
        ...
        <div className={styles.contentWrapper}>
          ...
          <div
            ...
            className={cn(
              ...
              activePrompt && styles.answerMode
            )}
          >
            {/* Answer Mode - When activePrompt is present */}
            {activePrompt ? (
              <div className={styles.answerModeContainer}>
                <div className={styles.answerModeHeader}>
                  <AlertCircle size={18} className={styles.answerModeIcon} />
                  <span className={styles.answerModeText}>
                    Answering {activePrompt.questions.length} question
                    {activePrompt.questions.length > 1 ? 's' : ''}
                  </span>
                </div>
                <div className={styles.answerModeActions}>
                  {activePrompt.allowSkip && (
                    <button
                      onClick={onSkipPrompt}
                      className={cn(styles.answerButton, styles.answerButtonSecondary)}
                    >
                      <SkipForward size={18} />
                      <span>Skip</span>
                    </button>
                  ...
                  <button
                    onClick={onSubmitPromptAnswers}
                    className={cn(styles.answerButton, styles.answerButtonPrimary)}
                  >
                    <CheckCircle size={18} />
              ...
              <>
                  ...
                  <div className={styles.filePreviewArea}>
                    {selectedFiles.map((file, index) => {
                      ...
                      const getDataFileIcon = () => {
                        if (isJSON)
                          return <FileJson size={24} color="var(--color-text-secondary)" />;
                        if (isCSV || isExcel)
                ...
                {/* Controls Row - Inside bordered container */}
                                ...
                                <div className={styles.dropdownModelList}>
                                  ...
                                  {(() => {
                                    ...
                                    const tieredModels = eligibleModels.filter(
                                      m => m.recommendation_tier && typeof m.recommendation_tier === 'object'
                                    );
                                    ...
                                    const sortedTiered = [...tieredModels].sort((a, b) => {
                                      if (!a.recommendation_tier || !b.recommendation_tier) return 0;
                                      const aTier = typeof a.recommendation_tier === 'object' ? a.recommendation_tier.tier : '';
                                      const bTier = typeof b.recommendation_tier === 'object' ? b.recommendation_tier.tier : '';
                                      return (tierPriority[aTier] || 999) - (tierPriority[bTier] || 999);
                                    });

                                    // Combine: tiered first (up to 5), then non-tiered (up to 5)
                                    const displayModels = [
                                      ...sortedTiered.slice(0, 5),
                                      ...nonTieredModels.slice(0, 5)
                                    ];
                                    ...
                                    >
                                      ...
                                      <div className={styles.dropdownItemContent}>
                                        <div className={styles.dropdownItemNameRow}>
                                          ...
                                          {model.recommendation_tier && typeof model.recommendation_tier === 'object' && (
                                            <span
                                              className={styles.tierIndicator}
                                              title={model.recommendation_tier.label}
                                              style={{
                                                backgroundColor: getTierColor(model.recommendation_tier.tier)
                                              }}
                                            />
                                          )}
                                        ...
                                        {(model.description || (model.recommendation_tier && typeof model.recommendation_tier === 'object' && model.recommendation_tier.description)) && (
